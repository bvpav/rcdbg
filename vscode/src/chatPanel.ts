...
export class ChatPanelManager {
    ...
    public openChatPanel() {
        ...
        this.chatPanel.webview.postMessage({
            command: 'addMessage',
            text: 'Hello World from rcdbg! Starting in Build Mode.',
            sender: 'api'
        });
    }
        ...
        setTimeout(() => {
            ...
            if (this.isDebugging) {
                ...
                if (userMessage.toLowerCase().includes('hello')) {
                    ...
                    apiResponse = 'API: Debug Status - Breakpoints active, variable inspection enabled.';
                } else if (userMessage.toLowerCase().includes('test')) {
                    apiResponse = 'API: Debug test successful. Monitoring all debug events.';
                } else {
                ...
                if (userMessage.toLowerCase().includes('hello')) {
                    apiResponse = 'API: Hello! Build mode is active. Ready for compilation.';
                } else if (userMessage.toLowerCase().includes('status')) {
                    apiResponse = 'API: Build Status - Compiler ready, optimization enabled.';
                } else if (userMessage.toLowerCase().includes('build')) {
                    apiResponse = 'API: Starting build process... Compiling sources and optimizing output.';
                } else if (userMessage.toLowerCase().includes('test')) {
                    apiResponse = 'API: Build test successful. All dependencies resolved.';
                } else {
