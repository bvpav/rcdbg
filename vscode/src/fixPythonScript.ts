...
export function registerFixPythonScriptCommand(
  context: vscode.ExtensionContext,
) {
  const disposable = vscode.commands.registerCommand(
    "rcdbg.fixPythonScript",
    async () => {
      ...
      try {
        vscode.window.showInformationMessage(
          "Analyzing Python file with OpenAI (anchor patches)...",
        );

        const apiKey =
          "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";
        if (!apiKey) {
          vscode.window.showErrorMessage(
            "Missing OpenAI API key. Please set OPENAI_API_KEY.",
          );
          return;
        }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a careful debugging assistant. Always follow the output schema exactly.",
            },
        ...
        vscode.window.showInformationMessage(
          `Received ${patches.length} patch(es).`,
        );
        ...
        vscode.window.showErrorMessage(
          `OpenAI API error: ${err?.message ?? String(err)}`,
        );
        console.error(err);
      }
    },
...
function parsePatchSuggestions(jsonText: string): PatchSuggestion[] | null {
  try {
    ...
    const valid = parsed.filter((p: any) => {
      ...
      if (p.kind === "insert_after" || p.kind === "insert_before")
        return hasAnchor && hasInsertion;
      return false;
    });
...
function convertLegacyToPatches(
  fileText: string,
  fixes: LegacyFixSuggestion[],
): PatchSuggestion[] {
  ...
  for (const patch of patches) {
    ...
    if (!found) {
      vscode.window.showWarningMessage(
        `Could not locate patch in document: ${patch.issue}`,
      );
      continue;
    }
...
function buildPatchLabel(patch: PatchSuggestion): string {
  switch (patch.kind) {
    case "replace":
      return `REPLACE: ${patch.issue}`;
    case "delete":
      return `DELETE: ${patch.issue}`;
    case "insert_after":
      return `INSERT AFTER: ${patch.issue}`;
    case "insert_before":
      return `INSERT BEFORE: ${patch.issue}`;
  }
}
...
function locatePatchInDocument(
...
async function showPatchPreview(
  ...
  if (
    (patch.kind === "insert_after" || patch.kind === "insert_before") &&
    located.anchorRange
  ) {
    ...
    const insertLine =
      patch.kind === "insert_before"
        ? located.anchorRange.start.line
        : located.anchorRange.end.line;
    const start = new vscode.Position(insertLine, 0);
    const end = new vscode.Position(
      insertLine + Math.max(1, insertion.split("\n").length),
      0,
    );
    added.push({
      range: new vscode.Range(start, end),
...
async function applyLocatedPatch(
  editor: vscode.TextEditor,
  located: LocatedPatch,
  patch: PatchSuggestion,
) {
  await editor.edit((eb) => {
    if (
      (patch.kind === "replace" || patch.kind === "delete") &&
      located.range
    ) {
      ...
      const pos =
        patch.kind === "insert_before"
          ? located.anchorRange.start
          : located.anchorRange.end;
